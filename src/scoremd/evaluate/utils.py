...
def once_on_every_and_all(
    trajectories: jnp.ndarray, f: Callable[[jnp.ndarray, Optional[int]], None], desc=None, call_on_all=True
):
    ...
    """
    num = trajectories.shape[0] + 1 if len(trajectories) > 1 else 1

    with tqdm(total=num, desc=desc) as pbar:
        for i, trajectory in enumerate(trajectories):
            f(trajectory, i)
            pbar.update(1)
        if call_on_all and len(trajectories) > 1:
