...
from scoremd.training.schedule import TrainingSchedule
...
def train(
    ...
    with tqdm(
        ...
        val_losses = []
        for relative_epoch, (state, loss, val_loss) in enumerate(pbar):
            epoch = relative_epoch + start_epoch  # relative epoch starts at 0, even when we continue training
...
def load_and_train(
    ...
    if continue_training and load_directory is None:
        raise ValueError("continue_training is True, but load_directory is None")
    ...
    if load_directory is not None:
        ...
        with ocp.CheckpointManager(os.path.abspath(os.path.expanduser(load_directory))) as checkpoint_manager:
            ...
            try:
                ...
                # this happens when the training system has different gpus than the one that saved the model
                log.error(f"Error restoring model: {e}")
                assert not continue_training, "Cannot continue training from a system with different hardware"
                log.info("We try to recover from this by loading the model without opt state")
