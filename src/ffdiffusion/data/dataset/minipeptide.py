...
from ffdiffusion.data.dataset.base import Datapoints, Dataset
import numpy as onp
...
if __name__ == "__main__":
    base_dir = "./storage/minipeptides"
    dataset = CGMinipeptideDataset(
        pdb_directory=os.path.join(base_dir, "pdbs"),
        train_path=os.path.join(base_dir, "train.npy"),
        # val_path=os.path.join(base_dir, "val.npy"),
        # test_path=os.path.join(base_dir, "test.npy"),
        limit_peptides=["MP"],
    )
    train, val, test = dataset._get_data()
    print(train.data.shape)
    print(train.peptides)
    print(train.peptide_lengths)

    dataset.write_animation(dataset.train.data[:100], "MP", "~/Downloads/MP_first.pdb")

    import matplotlib.pyplot as plt

    dataset.plot_phi_psi(dataset.train.data)
    plt.show()
